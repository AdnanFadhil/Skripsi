...
from another_utils_tes import declare_path, declare_vars, emd_decom_wavelet, get_integration_form, integrate, declare_LSTM_vars, Respective_LSTM_Testing, load_and_preprocess_data, predict_future, evl,plot_direction_accuracy, analyze_comparison_df
...
def run():
    st.title("📊 Stock Forecasting: ANTM.JK (2021–2025)")
    st.subheader("Fixed Forecasting Window: Jan 2021 – May 2025")
    ...
    st.session_state.end_date = '2025-05-31'
    st.session_state.ticker = "ANTM.JK"
    ...
    try:
        with st.status("🚀 Running forecasting pipeline...", expanded=True) as status:
            ...
            declare_LSTM_vars()
            # Automatically detect if models exist for all co-imf components
            required_imfs = [col for col in co_imfs.columns if col.startswith("co-imf")]
            model_paths = [os.path.join(MODEL_PATH, f"{base_ticker}_{imf}_model.h5") for imf in required_imfs]
            all_models_exist = all(os.path.exists(p) for p in model_paths)

            
            with st.spinner("Processing LSTM models..."):
                if all_models_exist:
                    st.success("📂 All LSTM models found — skipping training.")
                else:
                    st.info("🔁 Some models not found — training new models.")

                df_pred, model, scalarY = Respective_LSTM_Testing(
                    df=co_imfs,
                    base_ticker=base_ticker,
                    use_saved=all_models_exist
                )
        ...
        if backtest_enabled:
            ...
            print("DEBUG scalarY type before preprocess:", type(scalarY))
            print("🔍 scalarY type:", type(scalarY))
            print("🔍 scalarY content:", scalarY)

            series_full, series_scaled, _ = load_and_preprocess_data(
                csv_path,
                scalarY=scalarY,
                save_global_scaler=True,
                scaler_save_path=os.path.join(LOG_PATH, base_ticker)
            )

            print("DEBUG scalarY type after preprocess:", type(scalarY))
            future_preds = predict_future(
                model=model,
                last_data=series_scaled,
                days=30,
                scalerY=scalarY
            )
