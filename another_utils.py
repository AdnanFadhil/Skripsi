...
def emd_decom_wavelet(series=None, trials=10, re_decom=False, re_imf=0, draw=True, base_ticker=None):
    # If base_ticker is not provided, derive it from file_name
    dataset, file_name = check_dataset(series, input_form='series')
    series = dataset.values
    
    # Ensure base_ticker is set either from the argument or file_name
    if base_ticker is None:
        base_ticker = file_name.replace(".", "")  # Derive from file_name

    # Ensure base_ticker is properly set
    if not base_ticker:
        raise ValueError("Base ticker is not properly set. Ensure that file_name is valid.")

    # Print the base_ticker for debugging purposes
    print(f"Base Ticker: {base_ticker}")

    # Initialization of decomposition method
    ...
    # Ensure enough IMFs are produced
    required_imfs = 8
    if imfs_num < required_imfs:
        print(f"Decomposition produced {imfs_num} IMFs, increasing trials to generate more IMFs.")
        decom.trials = 50
        imfs_emd = decom(series)
        imfs_num = np.shape(imfs_emd)[0]
...
def LSTM_pred(data=None, draw=True, uni=False, show_model=True, train_set=None, next_pred=False, ahead=1, progress_callback=None):
    ...
    if draw and PERIODS != 0:
        ...
        # Show forecast plot in Streamlit
        st.pyplot(plt)
...
from tensorflow.keras.layers import Bidirectional, LSTM, Dropout, Dense, Attention, Input
from tensorflow.keras.models import Model
from tensorflow.keras.optimizers import Adam

def LSTM_model(shape):
    if LSTM_MODEL is None:
        print("🔹 Building LSTM Model with Attention Layer")
        
        # Input layer
        inputs = Input(shape=(shape[1], shape[2]))

        # Bidirectional LSTM Layer with increased units
        lstm_out = Bidirectional(LSTM(CELLS * 6, activation='relu', return_sequences=True))(inputs)
        lstm_out = Dropout(0.3)(lstm_out)

        # Attention Layer - query and value come from the same LSTM output
        attention_out = Attention()([lstm_out, lstm_out])  # Query and value are both lstm_out
        
        # No flattening: pass attention output directly to the next LSTM layer
        lstm_out2 = LSTM(CELLS * 4, activation='relu', return_sequences=True)(attention_out)
        lstm_out2 = Dropout(0.3)(lstm_out2)

        lstm_out3 = LSTM(CELLS * 2, activation='relu', return_sequences=True)(lstm_out2)
        lstm_out3 = Dropout(0.3)(lstm_out3)

        # Final LSTM Layer
        lstm_out4 = LSTM(CELLS, activation='relu', return_sequences=False)(lstm_out3)
        lstm_out4 = Dropout(0.3)(lstm_out4)

        # Output Layer
        output = Dense(1, activation='tanh')(lstm_out4)
...
def Respective_LSTM(df=None, draw=True, uni=False, show_model=True, next_pred=False, ahead=1):
    ...
    for i in range(df_len):
        ...
        test_pred, model, scalarY = LSTM_pred(
            ...
            next_pred=next_pred,
            ahead=ahead,
            progress_callback=epoch_update_callback
        )
...
def load_and_preprocess_data(ticker, scalarY=None):
    """
    Load ticker CSV from dataset folder, normalize the 'close' column, and return both the original and scaled series.

    Parameters:
    - ticker: str, stock ticker (e.g., 'BNII.JK')
    - scalarY: MinMaxScaler object (optional). If None, a new scaler is created and fitted.

    Returns:
    - series: Original unscaled pandas Series of 'close' prices.
    - series_scaled: Numpy array of scaled values.
    - scalarY: The fitted MinMaxScaler object.
    """
    dataset_path = os.path.join(DATASET_FOLDER, f"{ticker}")
    ...
    if 'Close' not in df.columns:
        raise ValueError(f"'close' column not found in {dataset_path}")
    ...
    if scalarY is None:
        ...
        series_scaled = scalarY.fit_transform(series.values.reshape(-1, 1))
    else:
        series_scaled = scalarY.transform(series.values.reshape(-1, 1))

    return series, series_scaled, scalarY

def predict_future(model, series_scaled, scalarY, days=7, lookback=30):
    """ Predict next 30 days using trained LSTM model """
    future_preds = []
    last_inputs = series_scaled[-lookback:].tolist()  # Get last known data

    for _ in range(days):
        X_input = np.array(last_inputs[-lookback:]).reshape((1, lookback, 1))
        next_pred = model.predict(X_input)[0][0]  # Predict one step ahead
        future_preds.append(next_pred)
        last_inputs.append([next_pred])  # Add to input for next prediction

    # De-normalize predictions
    future_preds = np.array(future_preds).reshape(-1, 1)
    future_preds = scalarY.inverse_transform(future_preds)
...
def Respective_LSTM_Testing(df=None, draw=True, uni=False, show_model=True, 
    ...
    - Fixed scaling handling
    - Protected IMF weight configuration 
    ...
    for imf_name in available_imfs:
        model_fn = {
            'co-imf0': LSTM_model_HighFreq,
            'co-imf1': LSTM_model_MidFreq,
            'co-imf2': LSTM_model_LowFreq
        }.get(imf_name, LSTM_model_MidFreq)
        
        # Train/predict for each IMF
    ...
    df_pred = pd.DataFrame(data_pred).T
    df_pred.columns = available_imfs
    
    # Combine IMFs into one prediction (still normalized)
    res_pred = pd.Series(np.zeros(len(df_pred)), index=df_pred.index)

    for imf_name, weight in IMF_WEIGHTS.items():
        imf_values = df_pred[imf_name].values.reshape(-1, 1)
        res_pred += imf_values.ravel() * weight

    # ===== 5. INVERSE SCALING =====
    # Inverse transform the combined prediction (mapping back to Close prices scale)
    if isinstance(scalarY, MinMaxScaler):
        res_pred = scalarY.inverse_transform(res_pred.values.reshape(-1, 1)).ravel()
    else:
        res_pred = res_pred * scalarY["scalar"] + scalarY["min"]

    # ===== 6. EVALUATION =====
    if PERIODS > 0:
        # Load actual 'Close' prices for comparison
        try:
        ...
        # Slice actual values for comparison
    ...
    if draw and PERIODS > 0 and df_evl is not None:
        # Plot individual IMF predictions
        fig, ax = plt.subplots(figsize=(12,4))
        for imf in df_pred.columns:
            ax.plot(df_pred[imf][:PERIODS], label=f"{imf} ({IMF_WEIGHTS[imf]*100:.1f}%)", alpha=0.7)
